...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="card p-6" id="quote-panel">
            ...
            @if($request->status === 'pending_courier_acceptance' && $activeQuote)
                ...
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="text-sm text-yellow-700">Quote was sent to
                            <strong>{{ $request->courier->first_name ?? 'Courier' }} {{ $request->courier->last_name ?? '' }}</strong>.
            ...
            @if(in_array($request->status, ['approved', 'assigned', 'pending_courier_acceptance']) || $request->is_price_quoted)
            <div class="mb-5">
                <div class="flex items-center justify-between mb-3">
                    <h4 class="font-semibold text-sm text-gray-700">Price Calculation</h4>
                    @if(!in_array($request->status, ['pending_courier_acceptance', 'accepted_by_courier', 'assigned']) && !in_array($request->status, ['picked_up', 'in_transit', 'delivered', 'completed']))
            ...
            {{-- Direct assign (no quote) --}}
            <div class="border border-gray-200 rounded-lg p-4 mt-3">
                <details>
                    <summary class="text-sm text-gray-500 cursor-pointer hover:text-gray-700 font-medium">
                        <i class="fas fa-user-check mr-2"></i>
                        Direct Assign (Skip Quote)
                    </summary>
                    <div class="mt-3">
                        <p class="text-xs text-orange-600 mb-3">
                            <i class="fas fa-exclamation-triangle mr-1"></i>
                            This bypasses the quote process. Courier will be directly assigned without price agreement.
                        </p>
                        <form action="{{ route('admin.requests.assign', $request) }}" method="POST">
                            ...
                            <div class="flex gap-2">
                                ...
                                <button type="submit"
                                    onclick="return confirm('Directly assign this courier without a price quote?')"
                                    class="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 text-sm whitespace-nowrap">
                                    <i class="fas fa-user-check mr-1"></i> Assign Directly
                                </button>
                            </div>
                        </form>
                    </div>
